...
const processPage = async (url, config, gotScraping, retryCount = 0) => {
    try { 
        ...
        if (error.message === 'socket hang up') {
            logger.warn(`Socket hang up error for url: ${url}, retrying...`);
            await sleep(config.retryDelay);
            return processPage(url, config, gotScraping, retryCount);
        }
        logger.error(`Found error scraping nsa process: ${error.message}`);
...
const processDateRange = async (config, fromDate, toDate, retryCount = 0) => {
    try {
        ...
        if (statusCode === 200) {
            console.log('Page exists, status code:', statusCode);
...
const scrape = async (config) => {
    try {
        ...
        const dateRanges = Helper.createDateRanges(sinceDate);
        logger.info(`Created ${dateRanges.length} date ranges to process`);
    
        for (const range of dateRanges) {
            // logger.info(`Processing date range: ${range.fromDate} to ${range.toDate}`);
